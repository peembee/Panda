...
export function NewProject({ sendUser }) {
  ...
  function checkInput() {
    if (
      ...
      newProject.status === "" ||
      newProject.status === "Enter Status"
    ) {
  ...
  const PostToSwagger = (e) => {
    // Function for post to swagger
    fetch(
      "https://axb22z45ygh20230227215753.azurewebsites.net/create-project",
      {
        method: "POST",
        headers: {
      ...
      .then((response) => {
        if (!response.ok) {
          ...
          setNewProject({
            // preventing duplicate from prev value
            projectName: null,
            description: null,
            status: null,
          });
  ...
  function PostToProjectList() {
    fetch(
      "https://axb22z45ygh20230227215753.azurewebsites.net/create-projectList",
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  function ActiveProjects() {
    // sorting api to get the name for the Active project user has
    let getMyProjects = "";
    ...
    for (let i = 0; i < projectList.length; i++) {
      if (projectList[i].fK_EmployeeId === myUniqueId) {
        for (let j = 0; j < swaggerData.length; j++) {
          //
          if (
            projectList[i].fK_ProjectId === swaggerData[j].projectId &&
            swaggerData[j].status === "Active"
          ) {
  ...
  function InActiveProjects() {
    // sorting api to get the name for the Inactive project user has
  ...
  return (
    <>
      <div className="bodyNewProject">
        <form>
          <div className="newProject">
            ...
            <div>
              <Button onClick={checkInput} className="btnAutoClear">
                Skapa
              </Button>
      ...
      <Modal show={show} backdrop="static">
        ...
        <Modal.Body>
          <div>Ett nytt Projekt har nu lagts till i projekt-listan..</div>
        </Modal.Body>
        <Modal.Footer>
